...
import { getUser, updateUser } from "../../services/user-requests";
...
export default function EditUser() {
    ...
    const buscaUsuario = async (id) => {
        const dadosUsuario = await (await getUser(id)).data;
        if (Object.keys(dadosUsuario).length === 0) {
            navigate('/usuario/view');
            mensagem('Não Encontrado!', 'Usuário não foi encontrado!', 'info');
        } else {
            setUsuario({
                ...usuario,
                ...dadosUsuario,
    ...
    const editarUsuario = async (event) => {
        ...
        for (const campo in usuario) {
            ...
            if (!['id', 'created_at', 'updated_at', 'deleted_at'].includes(campo)) {
                try {
        ...
        if (Object.keys(novosErros).length === 0) {
            ...
            setLoading(false);
            navigate('/usuario/view');
            mensagem("Alterado!", "Usuário alterado com sucesso!", "success");
        }
    ...
    return (
        <>
        <h2 className="card-title text-center mb-4"><span className="iconify-inline" data-icon="fluent:people-edit-24-regular"></span> Editar Usuário</h2>
        <div className="d-flex card border-2 border-info p-2 rounded-4  text-center col-md-8 offset-md-2 mb-5">
            <div className="card-body">
            <form className="row g-3" onSubmit={editarUsuario} noValidate>
                ...
                <div className="d-flex justify-content-center">
                    { loading ? <img src="/images/loading.gif" /> : <button type="submit" className="btn btn-primary"><span className="iconify-inline me-1" data-icon="fa-solid:user-edit"></span>Editar Usuário</button>}
                </div>
                </form>
            </div>
        </div>
